  ...
  return (
    <div>
      <div style={{ background:'var(--bg-secondary)', borderRadius:14, padding:'12px 16px', border:'1px solid var(--border-light)', marginBottom:16, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div>
          <div style={{ fontSize:11, color:'var(--text-secondary)', fontFamily:'inherit' }}>CREW TOTAL DEL PROYECTO</div>
      ...
      {!loaded && <div style={{ textAlign:'center', padding:'32px', color:'var(--text-muted)', fontFamily:'inherit' }}>Cargando crew...</div>}
        ...
        return (
          <div key={dk} style={{ marginBottom:20 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8, padding:'6px 0' }}>
              ...
              <span style={{ fontSize:10, color:'var(--text-secondary)', fontFamily:'inherit' }}>— {personas.length}</span>
            </div>
            {personas.map(p => (
              <div key={p.id} style={{ display:'flex', alignItems:'center', gap:12, background:'var(--bg-secondary)', borderRadius:12, padding:'10px 14px', marginBottom:6, border:`1px solid ${dm.color}20` }}>
                <div style={{ width:32, height:32, borderRadius:8, background:dm.color+'28', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  <Icon name="User" size={14} color={dm.color} />
                </div>
                <div style={{ flex:1 }}>
      ...
      {loaded && allCrew.length === 0 && (
        <div style={{ textAlign:'center', padding:'40px 20px', color:'var(--text-muted)', fontFamily:'inherit' }}>
          <div style={{ display:'flex', justifyContent:'center', marginBottom:12 }}><Icon name="Users" size={36} color="var(--text-secondary)" /></div>
