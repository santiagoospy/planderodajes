...
const StarRating = ({ value, onChange }) => <div style={{ display: 'flex', gap: 4 }}>{[1, 2, 3, 4, 5].map(n => <button key={n} onClick={() => onChange(n)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, fontSize: 24, color: n <= value ? '#f5a623' : '#e5e2dd' }}>★</button>)}</div>;
  ...
  return (
    <div style={{ background: 'var(--bg-secondary)', borderRadius: 12, border: `1px solid ${dept.color}33`, marginBottom: 8, overflow: 'hidden' }}>
      <div onClick={() => setOpen(!open)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', cursor: 'pointer', background: dept.color + '08' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: dept.color, fontFamily: 'inherit' }}>{dept.label}</div>
...
const AddScoutDeptModal = ({ onAdd, onClose }) => {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-secondary)', borderRadius: 18, padding: '22px 20px', width: '100%', maxWidth: 340 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 12 }}>
          {iconos.map(i => { const IconComponent = iconMap[i]; return (<button key={i} onClick={() => setIcon(i)} style={{ width: 36, height: 36, borderRadius: 8, border: 'none', cursor: 'pointer', background: icon === i ? color + '22' : '#f0ede8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><IconComponent size={16} color={icon === i ? color : 'var(--text-secondary)'} /></button>); })}
        </div>
...
const LocacionScoutCard = ({ loc, idx, onUpdate, onDelete, color }) => {
  ...
  return (
    <div style={{ background: 'var(--bg-secondary)', borderRadius: 14, border: `1px solid ${color}25`, marginBottom: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderBottom: collapsed ? 'none' : '1px solid #ede9e3', background: color + '06' }}>
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--bg-primary)', display: 'flex', flexDirection: 'column' }} className="slide-r">
      <div style={{ background: 'var(--bg-secondary)', padding: 'calc(env(safe-area-inset-top, 0px) + 14px) 20px 16px', borderBottom: '1px solid #ede9e3', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => setTab('locaciones')} style={{ flex: 1, fontFamily: 'inherit', fontSize: 12, fontWeight: 700, padding: '8px', borderRadius: 10, border: 'none', cursor: 'pointer', background: tab === 'locaciones' ? color : '#f0ede8', color: tab === 'locaciones' ? '#fff' : '#999' }}><MapPin size={13} style={{ display: 'inline', marginRight: 4 }} /> Locaciones</button>
          <button onClick={() => setTab('archivos')} style={{ flex: 1, fontFamily: 'inherit', fontSize: 12, fontWeight: 700, padding: '8px', borderRadius: 10, border: 'none', cursor: 'pointer', background: tab === 'archivos' ? color : '#f0ede8', color: tab === 'archivos' ? '#fff' : '#999' }}><Paperclip size={12} style={{ display: 'inline', marginRight: 4 }} /> Archivos</button>
        </div>
      </div>
